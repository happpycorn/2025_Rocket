...
# 這裡改成你的 CSV 檔名
csv_path = 'Data/RocketSim-WS0.csv'
...
# 確認欄位名稱
time = df['Time (s)']
cp_col = 'CP location (cm)'
cg_col = 'CG location (cm)'
...
ax1.plot(time, df[cp_col], label='CP location (cm)', color=cp_color, linewidth=2.5)
ax1.plot(time, df[cg_col], label='CG location (cm)', color=cg_color, linewidth=2.5)
